...
public class Products_Page extends JPanel {
	...
	public static void storeDataToTable()
	{
		...
		{
			Object[] dataProduct = {onTable.getPid(),onTable.getPname(),onTable.getPprice(),onTable.getPcate(),onTable.getPretailgood()};
	...
	private void updateDataProductToDatabase()
	{
		...
		{
			con = (com.mysql.jdbc.Connection) DriverManager.getConnection(DataMysqlConnection.getUrl(),DataMysqlConnection.getUser(),DataMysqlConnection.getPassword());
			String sqlacc = "UPDATE tblproducts SET proName = '"+ txtProductName.getText() +"', UnitPrice = "+Double.parseDouble(txtPriceUnit.getText())+
					",Category = '"+comboBox.getSelectedItem().toString()+"', Quantity = "+Integer.parseInt(txtRetailGoods.getText())+" WHERE ID = "+pid+"";
	...
	private void addDataProductToDatabase()
	{
		...
		{
			con = (com.mysql.jdbc.Connection) DriverManager.getConnection(DataMysqlConnection.getUrl(),DataMysqlConnection.getUser(),DataMysqlConnection.getPassword());
			String sqlacc = "INSERT INTO tblproducts values(null,'"+ txtProductName.getText() +"',"+Double.parseDouble(txtPriceUnit.getText())+",'"+comboBox.getSelectedItem().toString()+"',"+Integer.parseInt(txtRetailGoods.getText())+")";
			
			preparedStatement = (com.mysql.jdbc.PreparedStatement) con.prepareStatement(sqlacc);
		    preparedStatement.execute();
		    
		    Object[] newData= {txtProductID.getText(),txtProductName.getText(),txtPriceUnit.getText(),comboBox.getSelectedItem().toString(),txtRetailGoods.getText()};
