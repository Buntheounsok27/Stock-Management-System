...
public class AccountType_Page extends JPanel {
	...
	private void updateDataToDatabase() {
		...
		try {
			con = (com.mysql.jdbc.Connection) DriverManager.getConnection(DataMysqlConnection.getUrl(),
					DataMysqlConnection.getUser(), DataMysqlConnection.getPassword());
			String sqlacc = "Update tblaccounttype SET AccountType = '" + txtPosition.getText()
					+ "' where AccountType = '" + dListModel.getElementAt(lstCatSel) + "'";
			...
			Person_Accessable_Static.accessables.clear();
			Person_Accessable_Static.accTypeArr.clear();
			ReferenceMethod.updateAccTypeToDBWhenAccTypeUpdate(txtPosition.getText(),
