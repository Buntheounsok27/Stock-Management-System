...
import resource.GeaneralSetting_Accessable;
...
public class Cash_Register_Page extends JPanel implements ActionListener {
	...
	private JLabel lblSubtotal, lblShSubTotal, lblShCash, lblShTotal, lblShChange, lblShDate, lblShBy,lblShReceiptNo;
	public static JLabel lblShCustomer, lblShDiscount;

	private static double subtotal, total, change,cash,resubtotal;
	public static double discount;
	
	private String sale_date,sale_Time;
	...
	private void lblShCustomer_Clicked(MouseEvent eve, JFrame jf) {
		...
		customer_Selected.setDefaultCloseOperation(WindowConstants.DISPOSE_ON_CLOSE);
		jf.setEnabled(false);
	...
	private void btnAdd_Clicked(ActionEvent eve) {
		...
		if (!txtQty.getText().equals("")) {
			...
			Object[] datarow = { txtQty.getText(), dListModel.getElementAt(lstSel),decimalFormat.format(
					(Double.parseDouble(txtQty.getText())
							* Double.parseDouble(Product_Accessable_Static.getPpriceS()))) };
			dTableModel.addRow(datarow);
			Sale_Detail sale_Detail = new Sale_Detail();
			sale_Detail.setId(Sale_Detail.getIdN());
			sale_Detail.setSaleID(Sale.getIDN());
			...
			lblShSubTotal.setText(decimalFormat2.format(subtotal));

			dateTimeShow();
			DecimalFormat decimalFormat1 = new DecimalFormat("0000");
			
			lblShReceiptNo.setText(""+GeaneralSetting_Accessable.getReciept()+decimalFormat1.format(1));
			

		} else
			JOptionPane.showMessageDialog(null, "Please Enter Quality of item ...!", "Warning",
	...
	private void dateTimeShow() {
		...
		sale_date = simpleDateFormat.format(date);
		sale_Time = siFormat.format(time);
	...
	private void btnPay_Clicked(ActionEvent eve) {
		...
		{
			subtotal += Double.parseDouble(dTableModel.getValueAt(i, 2).toString());
		}
		lblShSubTotal.setText(decimalFormat2.format(subtotal));
		...
		lblShTotal.setText(decimalFormat2.format(total));
		lblShCustomer.setEnabled(false);
		tblInvioce.setEnabled(false);
	...
	private void btnCancelPay_Clicked(ActionEvent eve) {
		...
		lblShCustomer.setEnabled(true);
		tblInvioce.setEnabled(true);
	}
	...
	private void btnUpdateItem_Clicked(ActionEvent eve) {
		...
		DecimalFormat decimalFormat = new DecimalFormat("$   0.00");
		ReferenceMethod.getUnitPriceProductFromDB(dTableModel, rowTbSel);
		...
		details.set(rowTbSel,sale_Detail);
		dTableModel.setValueAt(decimalFormat1.format(resubtotal*Integer.parseInt(txtQty.getText())), rowTbSel,2);
		subtotal -= Double.parseDouble(dTableModel.getValueAt(rowTbSel, 2).toString());
		subtotal += resubtotal*Integer.parseInt(txtQty.getText());
		lblShSubTotal.setText(decimalFormat.format(subtotal));
	}
	...
	private void btnCashIn_Clicked(ActionEvent eve) {
		...
		if (!txtAmount.getText().equals("")) {
			...
			JOptionPane.showMessageDialog(null, "Please Enter Cash...!", "Warning", JOptionPane.PLAIN_MESSAGE);
	}
	...
	private void btnCheckOut_Clicked(ActionEvent eve) {
		...
		double cashin = cash - change;
		
		if(lblShCustomer.getText().equals("To Go"))
		{
			cusID = 1;
		}
		...
		try {
			...
			String sqlacc = "INSERT INTO tblsales values("+Integer.parseInt(Sale.getIDN())+",'"+sale_date+"','"+sale_Time+"',"+Integer.parseInt(Person_Accessable_Static.getAccID())+
					","+cusID+","+total+","+cashin+")";
		...
		for(Sale_Detail sold : details)
		{
			...
			try {
				con = (com.mysql.jdbc.Connection) DriverManager.getConnection(DataMysqlConnection.getUrl(),
					DataMysqlConnection.getUser(), DataMysqlConnection.getPassword());
				String sqlacc = "INSERT INTO tblsalesdetails values("+Sale_Detail.getIdN()+","+Integer.parseInt(sold.getSaleID())+","+Integer.parseInt(sold.getProID())+","+sold.getQty()+")";

				preparedStatement = (com.mysql.jdbc.PreparedStatement) con.prepareStatement(sqlacc);
				preparedStatement.executeUpdate();
				try {
					String sqlac = "SELECT Quantity FROM tblproducts WHERE ID = "+Integer.parseInt(sold.getProID());

					preparedStatement = (com.mysql.jdbc.PreparedStatement) con.prepareStatement(sqlac);
					resultSet = preparedStatement.executeQuery();
					if(resultSet.next())
					{
						int qtyold = resultSet.getInt(1);
						qtyupdate = qtyold - sold.getQty();
						try {
							String sqla = "UPDATE tblproducts SET Quantity = "+qtyupdate+" WHERE ID = "+Integer.parseInt(sold.getProID());

							preparedStatement = (com.mysql.jdbc.PreparedStatement) con.prepareStatement(sqla);
							preparedStatement.executeUpdate();
			...
			Sale_Detail.setIdN(String.valueOf(Integer.parseInt(Sale_Detail.getIdN())+1));
		}

		details.clear();
		tblInvioce.setEnabled(true);
		lblShCustomer.setText("To Go");
		lblShDiscount.setText("0.00");
		lblShReceiptNo.setText("");
		lblShCustomer.setEnabled(true);
	...
	private void rowData_Selected(MouseEvent eve) {
		...
		if (rowTbSel > -1) {
			...
			resubtotal = Double.parseDouble(dTableModel.getValueAt(rowTbSel, 2).toString())/Integer.parseInt(dTableModel.getValueAt(rowTbSel, 0).toString());
	...
	public Cash_Register_Page(JFrame jFrame) {
		...
		GeaneralSetting_Accessable.setReciept("POSIN-");
		GeaneralSetting_Accessable.setCompanyName("G5 Smart Shop");
		lblCompanyName.setText(GeaneralSetting_Accessable.getCompanyName());
	}
