...
public class Customer_Info_Page extends JPanel {
	...
	private void btnAdd_Clicked(ActionEvent eve)
	{
		...
		for(int i=0; i<dTableModel.getRowCount();i++)
		{
			...
			{
				same = true;
				JOptionPane.showMessageDialog(null, "This Phone Number have Already...!", "Adding New Data ", JOptionPane.PLAIN_MESSAGE);
				break;
			}
			else
			{
	...
	private void btnUpdate_Clicked(ActionEvent eve)
	{
		...
		for(int i=0; i<dTableModel.getRowCount();i++)
		{
			...
			{
				same = true;
				JOptionPane.showMessageDialog(null, "This Phone Number have Already...!", "Updating Data ", JOptionPane.PLAIN_MESSAGE);
				break;
			}
			else
			{
	...
	private void updateDataProductToDatabase()
	{
		...
		{
			con = (com.mysql.jdbc.Connection) DriverManager.getConnection(DataMysqlConnection.getUrl(),DataMysqlConnection.getUser(),DataMysqlConnection.getPassword());
			String sqlacc = "UPDATE tblcustomers SET CusName = '"+ txtCustomerName.getText() +"', Phone = '"+(txtCustPhone.getText())+
					"', Address = '"+txtAddress.getText()+"', Country_Region = '"+txtCountry.getText()+"', MemberType= '"+comboBox.getSelectedItem().toString()+"' WHERE CusID = "+pid+"";
	...
	private void addDataProductToDatabase()
	{
		...
		{
			con = (com.mysql.jdbc.Connection) DriverManager.getConnection(DataMysqlConnection.getUrl(),DataMysqlConnection.getUser(),DataMysqlConnection.getPassword());
			String sqlacc = "INSERT INTO tblcustomers values(null,'"+ txtCustomerName.getText() +"','"+txtCustPhone.getText()+"','"+txtAddress.getText()+"','"+txtCountry.getText()+"','"+comboBox.getSelectedItem().toString()+"',0)";
			
			preparedStatement = (com.mysql.jdbc.PreparedStatement) con.prepareStatement(sqlacc);
		    preparedStatement.execute();
		    
		    Object[] newData= {txtCustomerName.getText(),txtCustPhone.getText(),comboBox.getSelectedItem().toString()};
	...
	private void rowOnTblProduct_Selected(MouseEvent eve)
	{
		...
		if(rowSel>-1){
			...
			{
				con = (com.mysql.jdbc.Connection) DriverManager.getConnection(DataMysqlConnection.getUrl(),DataMysqlConnection.getUser(),DataMysqlConnection.getPassword());
				String sqlacc = "SELECT tblcustomers.CusID,tblcustomers.CusName,tblcustomers.Phone,tblcustomers.Address,tblcustomers.Country_Region,tblmember.MemberType FROM tblcustomers INNER JOIN tblmember ON tblcustomers.MemberType = tblmember.MemberType WHERE Phone = '"+dTableModel.getValueAt(rowSel, 1)+"'";
