...
public class Member_Page extends JPanel {
	...
	private void storeDataToList()
	{
		...
		{
			Object[] rowdata = {dolist.getMemberType(),dolist.getDiscount()};
	...
	private void btnAdd_Clicked(ActionEvent eve)
	{
		...
		for(int i=0; i<dListModel.getRowCount();i++)
		{
			...
			{
				same = true;
				JOptionPane.showMessageDialog(null, "This Member Type have Already...!", "Adding New Data ", JOptionPane.PLAIN_MESSAGE);
				break;
			}
			else
			{
	...
	private void btnUpdate_Clicked(ActionEvent eve)
	{
		...
		for(int i=0; i<dListModel.getRowCount();i++)
		{
			...
			{
				same = true;
				JOptionPane.showMessageDialog(null, "This Member have Already...!", "Adding New Data ", JOptionPane.PLAIN_MESSAGE);
				break;
			}
			else
			{
	...
	private void insertDataToDatabase()
	{
		...
		try
		{
			...
			String sqlacc = "INSERT INTO tblmember values(null,'"+txtMemberType.getText()+"',"+Float.parseFloat(txtDiscount.getText())+",0)";
	...
	private void updateDataToDatabase()
	{
		...
		try
		{
			...
			String sqlacc = "Update tblmember SET MemberType = '" +txtMemberType.getText() +"', Discount = "+Double.parseDouble(txtDiscount.getText())+" where MemberType= '"+dListModel.getValueAt(lstCatSel, 0)+"'";
			...
			Member_Accessable.member_Accessables_Arr.clear();
			ReferenceMethod.updateMemberTypeToDBWhenMemberTypeUpdate(txtMemberType.getText(),dListModel.getValueAt(lstCatSel, 0).toString());
